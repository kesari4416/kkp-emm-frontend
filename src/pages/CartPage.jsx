...
export default function CartPage() {
  ...
  return (
    <div data-testid="cart-page" className="max-w-[1480px] mx-auto px-4 md:px-8 py-10 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10">
      <div>
        <div className="eyebrow text-[var(--text-mute)]">Your bag</div>
        <h1 className="font-display font-black text-4xl mt-2 mb-8">{items.length} {items.length === 1 ? "item" : "items"}</h1>

        <div className="divide-y hairline border-t border-b hairline">
          {items.map((it) => (
            <div key={it.cart_item_id} data-testid={`cart-item-${it.cart_item_id}`} className="py-6 flex gap-4">
              <Link to={`/product/${it.product_id}`} className="w-28 aspect-[3/4] flex-shrink-0 bg-[var(--bg-admin)]">
                <img src={it.product?.images?.[0]} alt={it.product?.name} className="w-full h-full object-cover" />
              </Link>
              <div className="flex-1 flex flex-col">
                <div className="eyebrow text-[var(--text-mute)]">{it.product?.brand}</div>
                <Link to={`/product/${it.product_id}`} className="text-sm font-medium hover:text-[var(--accent)]">
                  {it.product?.name}
                </Link>
      ...
      <aside className="fk-card rounded p-6 h-fit sticky top-24">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="border-t hairline pt-3 mt-3 flex justify-between font-display font-bold text-lg">
        ...
        <Button
          ...
          className="w-full mt-6 btn-buy rounded-sm py-6 eyebrow"
        >
          Proceed to checkout
        </Button>
