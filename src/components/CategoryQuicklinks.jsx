  ...
  return (
    <section className="max-w-[1480px] mx-auto px-4 md:px-8 py-7 md:py-10">
      <div className="mb-4 md:mb-5">
        ...
        <h2 className="font-display font-black text-xl sm:text-2xl md:text-3xl tracking-tight mt-1">
          Trending categories
        </h2>
      </div>
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2.5 md:gap-5">
        {items.map((c) => (
          <Link
            key={c.key}
            to={c.to}
          ...
          >
            ...
            <div className="mt-2 text-[11px] sm:text-xs md:text-sm font-semibold group-hover:text-[var(--brand-pink)] transition-colors leading-tight line-clamp-2">{c.title}</div>
          </Link>
        ))}
      </div>
    </section>
